...
export function createExperimentMode({world,trialEngine,getParameters=()=>({}),onTrial=()=>{},onState=()=>{}}={}){
  ...
  function thresholdEstimate(){if(reversals.length<4)return null;const xs=reversals.slice(-8).map(x=>x.ratio).filter(x=>x>0);if(!xs.length)return null;return Math.exp(xs.reduce((s,x)=>s+Math.log(x),0)/xs.length);}
  ...
  function forgivingPick(e){
    ...
    // Only a thin halo around the visible object counts. Background clicks must
    // stay background clicks and must not advance the experiment.
    ...
    for(const o of objects){
      ...
      const halo=clamp(s.r*.12,6,14);
      const hitR=s.r+halo;
      if(d<=hitR&&d<bestDistance){bestDistance=d;best=o;}
    }
    return best;
  }
  ...
  function updateStaircase(outcome){
    ...
    if(outcome==='correct'){
      ...
      correctStreak=0;currentRatio=clamp(currentRatio*STEP_FACTOR,MIN_RATIO,MAX_RATIO);direction='up';
    }
  ...
  function answerUncertain(){if(!active)return;const result=trialEngine.uncertain();if(result.type==='none')return;complete('uncertain',result,null);}
