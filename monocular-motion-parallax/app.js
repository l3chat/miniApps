...
function setExperimentLock(locked){for(const id of ['baseline','frequency','focusDistance','waveform','fov','sceneDepth'])if($(id))$(id).disabled=locked;document.querySelectorAll('#modeButtons button').forEach(b=>b.disabled=locked);$('sceneBtn').disabled=locked;}
...
function restoreSettings(){
  ...
  cameraMotion.set('mode',c.mode??'static');cameraMotion.set('baselineCm',Number(c.baselineCm??8));cameraMotion.set('frequency',Number(c.frequency??1.6));cameraMotion.set('focusDistance',Number(c.focusDistance??VIEWING_DISTANCE_M));cameraMotion.set('waveform',c.waveform??'sine');
...
$('resetBtn').onclick=()=>{if(training.isActive())training.stop('reset');if(experiment.isActive())experiment.stop();setExperimentLock(false);cameraMotion.reset();const defaults={baseline:8,frequency:1.6,focusDistance:VIEWING_DISTANCE_M,fov:55,sceneDepth:2.4};for(const [id,v] of Object.entries(defaults)){if($(id)){ $(id).value=v;$(id+'Val').textContent=id==='fov'?String(v):Number(v).toFixed(1); }}$('waveform').value='sine';world.setFov(55);world.setSceneDepth(2.4);fit();world.buildScene();trialEngine.reset();document.querySelectorAll('#modeButtons button').forEach(x=>x.classList.toggle('active',x.dataset.mode==='static'));$('modeText').textContent=modeLabel();saveSettings({lastAppMode:'training'});updateTrainingButton();updateExperimentStatus({active:false,trialNo:0,total:0,threshold80:null});};
addEventListener('keydown',e=>{if(e.code==='Space'){e.preventDefault();$('pauseBtn').click();}if(e.key.toLowerCase()==='f')$('fullscreenBtn').click();});

restoreSettings();
fit();
cameraMotion.update(0);
world.buildScene();
applyLanguage(lang,{persist:false});saveSettings();updateExperimentStatus(lastExperimentState);
